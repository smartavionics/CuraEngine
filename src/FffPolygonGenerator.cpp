        ...
        for (unsigned int layer_nr = 0; layer_nr < mesh.layers.size(); layer_nr++)
        {
            ...
            for (SliceLayerPart& part : layer.parts)
            {
                ...
                if (mesh.settings.get<bool>("fill_outline_gaps"))
                {
                    ...
                    if (outline_gaps.size() > 0 && inner.size() > 0)
                    {
                        // shrink outline gaps a tiny amount to remove any little spikes that can be created where the thin wall meets the normal wall
                        outline_gaps = outline_gaps.offset(-wall_line_width_0 * 0.1);
                        // expand outline gaps slightly to avoid gaps where the thin walls meet the normal wall
                        outline_gaps = outline_gaps.offset(wall_line_width_0 / 2).intersection(outer);
                    }
