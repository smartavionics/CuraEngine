...
void FffPolygonGenerator::processOozeShield(SliceDataStorage& storage)
{
    ...
    for (int layer_nr = 0; layer_nr <= storage.max_print_height_second_to_last_extruder; layer_nr++)
    {
        ...
        storage.oozeShield.push_back(storage.getLayerOutlines(layer_nr, around_support, around_prime_tower).offset(ooze_shield_dist, Clipper2Lib::JoinType::Round).getOutsidePolygons());
    }
