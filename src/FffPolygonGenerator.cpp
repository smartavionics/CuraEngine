...
void FffPolygonGenerator::processBasicWallsSkinInfill(SliceDataStorage& storage, const size_t mesh_order_idx, const std::vector<size_t>& mesh_order, ProgressStageEstimator& inset_skin_progress_estimate)
{
    ...
    if (mesh.settings.get<bool>("meshfix_remove_holes_above_air"))
    {
        for (size_t layer_number = 1; layer_number < mesh.layers.size(); layer_number++)
        {
            ...
            {
                if (!a_mesh.settings.get<bool>("anti_overhang_mesh") && a_mesh.layers.size() >= layer_number)
                {
        ...
        for (LayerIndex layer_nr = 0; layer_nr < static_cast<LayerIndex>(mesh.layers.size()); layer_nr++)
        {
            ...
            for (SliceLayerPart& part : layer.parts)
            {
                ...
                if (fill_gaps_between_inner_wall_and_skin_or_infill && part.insets.size() > 0)
                {
                    ...
                    Polygons inner;
                    bool have_roofing = false;
                    for (const SkinPart& skin_part : part.skin_parts)
                    {
                        inner.add(skin_part.outline);
                        have_roofing |= !skin_part.roofing_fill.empty();
                    }
                    ...
                    const bool zigzag_pattern = (have_roofing ? mesh.settings.get<EFillMethod>("roofing_pattern") : (layer_nr == 0) ? mesh.settings.get<EFillMethod>("top_bottom_pattern_0") : mesh.settings.get<EFillMethod>("top_bottom_pattern")) == EFillMethod::ZIG_ZAG;
                    if (zigzag_pattern)
                    {
...
void FffPolygonGenerator::processInsets(SliceMeshStorage& mesh, size_t layer_nr)
{
    ...
    {
        WallsComputation walls_computation(mesh, layer_nr);
