...
void WallsComputation::generateInsets(SliceLayerPart* part)
{
    ...
    const bool is_top_layer = (unsigned)(layer_nr + 1) >= mesh.layers.size() || mesh.partLayerIsTopmost(*part, layer_nr);
    size_t inset_count = (settings.get<size_t>("wall_line_count") > 0) && ((single_inset_top && is_top_layer) || (single_inset_bottom && layer_nr == 0)) ? 1 : settings.get<size_t>("wall_line_count");
    ...
    for(size_t i = 0; i < inset_count; i++)
    {
        ...
        else if (i == 1)
        {
            ...
            if (single_inset_top)
            {
                ...
                const coord_t skin_expansion = line_width_x;
                ...
                part->insets[1].removeSmallAreas(2 * INT2MM(line_width_x) * INT2MM(line_width_x));
            }
        }
        else
        {
        ...
        if (i == 0)
        {
            ...
            int wall_extruder_nr = settings.get<int>("wall_0_extruder_nr");
            ...
            part->insets[0].simplify(maximum_resolution, maximum_deviation);
        }
        ...
        if (i == 0)
        {
            ...
            {
                part->print_outline = part->insets[0].offset(line_width_0 / 2, Clipper2Lib::JoinType::Square);
            }
            else
            {
                part->print_outline = part->outline;
