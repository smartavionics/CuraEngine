...
#pragma omp critical
    ...
    for (const SliceMeshStorage& mesh : storage.meshes)
    {
        ...
        if (layer_nr == 0)
        {
            ...
            inner_wall_width *= train.settings.get<Ratio>("initial_layer_line_width_factor");
        }
        max_inner_wall_width = std::max(max_inner_wall_width, inner_wall_width);
    }
...
std::vector<size_t> FffGcodeWriter::getUsedExtrudersOnLayerExcludingStartingExtruder(const SliceDataStorage& storage, const size_t start_extruder, const LayerIndex& layer_nr) const
{
    ...
    if (mesh_group_settings.get<bool>("prime_tower_enable") && !mesh_group_settings.get<bool>("prime_tower_compact") && layer_nr >= 0 && layer_nr <= storage.max_print_height_second_to_last_extruder)
    {
        ...
        */
        ...
        if (hasSkinEdgeSupport)
        {
            ...
            if (!infill_below_skin_per_density.empty())
            {
                ...
                {
                    Infill infill_comp(pattern, zig_zaggify_infill, connect_polygons, infill_below_skin_per_density, outline_offset,
                                       infill_line_width, infill_line_distance_here, infill_overlap, infill_multiplier,
                                       infill_angle, gcode_layer.z / mesh.settings.get<Ratio>("infill_scaling_z"), infill_shift, max_resolution, max_deviation, skin_below_wall_count, infill_origin,
                                       perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count,
                                       pocket_size, pattern_resolution, infill_wave_amplitude, infill_wave_wavelength);
...
void FffGcodeWriter::processSpiralizedWall(const SliceDataStorage& storage, LayerPlan& gcode_layer, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part, const SliceMeshStorage& mesh) const
{
    ...
    const bool is_bottom_layer = (layer_nr == static_cast<LayerIndex>(mesh.settings.get<size_t>("initial_bottom_layers")));
    ...
    if (extruder_nr == skin_extruder_nr)
    {
        for (Polygons skin_perimeter : skin_part.insets)
        {
            if (skin_perimeter.size() > 0)
            {
                ...
                setExtruder_addPrime(storage, gcode_layer, extruder_nr);
                gcode_layer.setIsInside(true); // going to print stuff inside print object
                const GCodePathConfig& skin_config = (skin_part.roofing_fill.empty()) ? mesh_config.skin_config : mesh_config.roofing_config;
                gcode_layer.addWalls(skin_perimeter, mesh, skin_config, mesh_config.bridge_skin_config, nullptr); // add polygons to gcode in inward order
            }
        }
    }
}
...
void FffGcodeWriter::processRoofing(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SkinPart& skin_part, Polygons& concentric_perimeter_gaps, bool& added_something) const
{
    ...
    const coord_t skin_overlap = mesh.settings.get<coord_t>("roofing_overlap_mm");
...
void FffGcodeWriter::processTopBottomWithBridges(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SkinPart& skin_part, Polygons& concentric_perimeter_gaps, bool& added_something) const
{
    ...
    for (unsigned n = 0; n < bridge_regions.size(); ++n)
    {
        ...
        const coord_t bridge_skin_expansion = std::max(MM2INT(0.01), mesh.settings.get<coord_t>((n > 1) ? "bridge_skin_overlap_mm_3" : (n > 0) ? "bridge_skin_overlap_mm_2" : "bridge_skin_overlap_mm"));

        Polygons bridge_skin = bridge_regions[n].intersection(skin_part.outline).offset(std::min(max_bridge_skin_expansion, bridge_skin_expansion));
    ...
    if (skin_polygons.size() > 0 || skin_lines.size() > 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                constexpr coord_t wipe_dist = 0;
                gcode_layer.addLinesByOptimizer(skin_lines, config, space_fill_type, enable_travel_optimization, wipe_dist, flow, near_start_location, fan_speed, avoid_freq, &mesh, pattern);
            }
        }
    }
}
...
void FffGcodeWriter::processPerimeterGaps(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const Polygons& perimeter_gaps, const GCodePathConfig& perimeter_gap_config, bool& added_something) const
{
    if (true)
    {
        ...
        fillNarrowGaps(storage, gcode_layer, mesh, extruder_nr, perimeter_gaps.difference(gcode_layer.getBridgeWallMask()), perimeter_gap_config, is_outline, added_something);
        return;
    }
...
void FffGcodeWriter::setExtruder_addPrime(const SliceDataStorage& storage, LayerPlan& gcode_layer, const size_t extruder_nr) const
{
    ...
    // first layer again.
    if ((prime_all || ((gcode_layer.getLayerNr() > 0) && extruder_changed) || ((gcode_layer.getLayerNr() == 0) && storage.primeTower.multiple_extruders_on_first_layer)) || (extruder_nr == outermost_prime_tower_extruder))
    {
