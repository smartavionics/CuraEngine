...
public:
    ...
    */
     ...
     * \return the added extrusion path or nullptr if the path was not created because it was shorter than the minimum path length
     */
    GCodePath *addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, const Ratio& flow = 1.0_r, bool spiralize = false, Ratio speed_factor = 1.0_r, double fan_speed = GCodePathConfig::FAN_SPEED_DEFAULT);
     ...
     * \param line_start_offset The length of the wall before this line is printed
     * \param bridge_start_offsets The start offsets of the bridges in the current wall
     ...
     * \param pattern (optional) the fill pattern used
     */
    void addLinesByOptimizer(const Polygons& polygons, const GCodePathConfig& config, const SpaceFillType space_fill_type, const bool enable_travel_optimization = false, const coord_t wipe_dist = 0, const Ratio flow_ratio = 1.0, const std::optional<Point> near_start_location = std::optional<Point>(), const double fan_speed = GCodePathConfig::FAN_SPEED_DEFAULT, const float avoid_freq = 0, const SliceMeshStorage* mesh = nullptr, const EFillMethod pattern = EFillMethod::LINES);
