...
#include "infill/DiscreteLinesInfill.h"
    ...
    case EFillMethod::SKIN_WAVE_TRIANGLE:
        {
            ...
            if (mesh && (pattern == EFillMethod::WAVE_SINE || pattern == EFillMethod::WAVE_TRIANGLE) && mesh->infill_angles.size() == 2)
            {
                ...
                std::vector<double> infill_angles = mesh->settings.get<std::vector<double>>("infill_angles");
                def += ",\"angle\": [";
