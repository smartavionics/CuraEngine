...
, wall_line_count((mesh.settings.get<size_t>("wall_line_count") > 0) &&
                  ((mesh.settings.get<bool>("only_one_wall_bottom") && layer_nr == 0) ||
                   (mesh.settings.get<bool>("only_one_wall_top") &&
                    ((unsigned)(layer_nr + 1) >= mesh.layers.size() || mesh.layers[layer_nr + 1].parts.empty()))) ? 1 : mesh.settings.get<size_t>("wall_line_count"))
...
void SkinInfillAreaComputation::generateSkinAndInfillAreas(SliceLayerPart& part)
{
    ...
    if (part.insets.size() > 1 && mesh.settings.get<bool>("only_one_wall_top"))
    {
        ...
        // shrink/expand by skin_line_width*3/4 to remove areas narrower than 1.5 * skin_line_width
        upskin.add(part.insets[0].offset(-(skin_line_width*3/4 + wall_line_width_0/2 + 10)).difference(part.insets[1].offset(wall_line_width_x/2 + 10)).offset(skin_line_width*3/4));
    }
